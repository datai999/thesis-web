...
const MainComponent = () => {
  ...
  if (window.location.pathname === "/refer-topics") {
    history.push(`/refer-topics/outline/${context.semester.name}`);
  }
  const isThesisTab = window.location.pathname.match("/refer-topics/thesis")
    ? true
    : false;
  ...
  const pushTabIndex = (index) => {
    history.push(
      `/refer-topics/${index === 1 ? "thesis" : "outline"}/${
