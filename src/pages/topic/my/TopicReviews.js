...
const fields = [
  ...
  { key: "names", label: "Tên đề tài", _style: { width: "50%" } },
  ...
  {
    key: "guideTeacherCodeNames",
    label: "Giáo viên hướng dẫn",
    _style: { width: "25%" },
  },
  {
    key: "guideTeacherEmails",
    label: "Email giáo viên hướng dẫn",
    _style: { width: "25%" },
  },
  {
    ...
    api
      ...
      .then((response) => {
        response.forEach((element) => {
          element.guideTeacherCodeNames = element.guideTeachers.map(
            (teacher) =>
              teacher.code + " " + teacher.firstName + " " + teacher.lastName
          );
          element.guideTeacherEmails = element.guideTeachers.map(
      ...
      <CDataTable
        ...
        scopedSlots={{
          ...
          guideTeacherCodeNames: (item) =>
            multiLine(
              item.guideTeachers.map((teacher) => (
                <div>
                  {teacher.code} {teacher.firstName} {teacher.lastName}
                </div>
              ))
            ),
          guideTeacherEmails: (item) => multiLine(item.guideTeacherEmails),
